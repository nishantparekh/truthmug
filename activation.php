...
if (isset($_GET['u']) && isset($_GET['e']) && isset($_GET['p'])) {
	...
	$sql = "SELECT * FROM users WHERE username='$u' AND email='$e' AND password='$p' LIMIT 1";
	...
	$sql = "UPDATE users SET activated='1' WHERE username='$u' AND email='$e' AND password='$p'  LIMIT 1";
    $query = mysqli_query($db_conx, $sql);
	// Optional double check to see if activated in fact now = 1
	$sql = "SELECT * FROM users WHERE username='$u' AND email='$e' AND activated='1' LIMIT 1";
    $query = mysqli_query($db_conx, $sql);
	$numrows = mysqli_num_rows($query);
